<x-admin::layouts>
    <x-slot:title>
        ...
        @pushOnce('scripts')
            ...
            <script type="module">             @if (!empty($users) && $users->isNotEmpty())             window.dashboardUsers = @json($users);             @endif
                 @if (!empty($managers))             window.dashboardManagers = @json($managers);             @endif
                 app.component('v-dashboard-filters', {                 template: '#v-dashboard-filters-template',
                     data() {                     return {                         filters: {                             user_id: @json($defaultUserId ? [$defaultUserId] : []),                             manager_email: '',                             start: "{{ $startDate->format('Y-m-d') }}",                             end: "{{ $endDate->format('Y-m-d') }}",                         },                         showUserDropdown: false,                         showManagerDropdown: false,                         users: window.dashboardUsers || [],                         managers: window.dashboardManagers || [],                     }                 },
                     computed: {                     selectedUsersLabel() {                         if (!this.filters.user_id || this.filters.user_id.length === 0) {                             return @json(__('All Users'));                         }
                             if (this.filters.user_id.length === 1) {                             const userId = this.filters.user_id[0];                             const user = this.users.find(u => u.id == userId);                             return user ? user.name : '1 User';                         }
                             return this.filters.user_id.length + " Users";                     },
                         selectedManagerLabel() {                         if (!this.filters.manager_email) {                             return @json(__('All Managers'));                         }
                             const manager = this.managers.find(m => m.email === this.filters.manager_email);                         return manager ? manager.name : @json(__('Manager'));                     }                 },
                     mounted() {                     // Close dropdown on click outside                     document.addEventListener('click', this.handleClickOutside);                 },
                     unmounted() {                     document.removeEventListener('click', this.handleClickOutside);                 },
                     methods: {                     toggleUserDropdown() {                         this.showUserDropdown = !this.showUserDropdown;                         this.showManagerDropdown = false;                     },
                         toggleManagerDropdown() {                         this.showManagerDropdown = !this.showManagerDropdown;                         this.showUserDropdown = false;                     },
                         handleClickOutside(event) {                         const userDropdown = this.$refs.userDropdown;                         if (userDropdown && !userDropdown.contains(event.target)) {                             this.showUserDropdown = false;                         }
                             const managerDropdown = this.$refs.managerDropdown;                         if (managerDropdown && !managerDropdown.contains(event.target)) {                             this.showManagerDropdown = false;                         }                     },
                         selectManager(email) {                         this.filters.manager_email = email;                         this.showManagerDropdown = false;                     },
