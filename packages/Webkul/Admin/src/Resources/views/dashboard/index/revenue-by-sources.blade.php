...
@pushOnce('scripts')
    ...
    <template v-else>
        <div class="grid gap-4 rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
            ...
            <!-- Doughnut Chart / Progress Card -->
            <div class="flex w-full max-w-full flex-col gap-4 px-8 pt-4" v-if="report.statistics.length">
                <template v-if="report.statistics[0].meta !== undefined">
                    <!-- BigQuery Progress Card -->
                    <div class="flex flex-col gap-6 py-4">
                        <div class="flex justify-between items-end">
                            <div class="flex flex-col gap-1">
                                ...
                                <p class="text-3xl font-bold text-gray-900 dark:text-gray-100">
                                    @{{ report.statistics[0].formatted_total }}
                                </p>
                            </div>
                            <div class="text-right">
                                <p class="text-sm font-medium text-gray-500">
                                    Meta: @{{ report.statistics[0].formatted_meta }}
                                </p>
                                <p class="text-2xl font-bold"
                                    :class="report.statistics[0].percentage >= 100 ? 'text-green-600' : 'text-blue-600'">
                                    @{{ Math.round(report.statistics[0].percentage) }}%
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="relative pt-1">
                            <div class="w-full h-6 rounded-full bg-gray-600 dark:bg-gray-700 overflow-hidden">
                                <div :style="'width: ' + Math.min(report.statistics[0].percentage, 100) + '%; height: 100%;'"
                                    :class="[
                                            'rounded-full transition-all duration-500',
                                            report.statistics[0].percentage >= 100 ? 'bg-green-500' : 'bg-blue-500'
                                        ]">
                                </div>
                            </div>
                            <div v-if="report.statistics[0].percentage > 100"
                                class="mt-2 text-xs font-semibold text-green-500">
                                Meta Superada!
                            </div>
                        </div>
                    </div>
